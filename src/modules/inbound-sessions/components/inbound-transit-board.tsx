...
export function InboundTransitBoard() {
  ...
  async function handleCreateReceiving(truckId: number) {
    try {
      const session = await createReceivingMutation.mutateAsync(truckId);
      toast.success(
            ...
            render: (row) => (
              <div className="flex flex-wrap items-center justify-center gap-2">
                <Button
                  ...
                  onClick={() => void handleCreateReceiving(row.truckId)}
                >
