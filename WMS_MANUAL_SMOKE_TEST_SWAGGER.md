...
- `exceptionScanCount` > 0 إذا وُجدت مسوحات `UNKNOWN` / `WRONG_POSITION` / …
...
`POST /api/v1/dealer/shipment-requests/{deliveryOrderId}/x`— `DEALER_TOKEN`
...
### 10.5 إنشاء جلسات Picking مقترحة
...
**متوقّع** `200`**:** جلسة `PENDING_APPROVAL` مع `outboundRequests`.
...
### 10.10 مسح الالتقاط (موبايل — OS-4)

`GET /api/v1/wms/my-warehouse/tasks/picking` — `STAFF_TOKEN`

`GET /api/v1/wms/my-warehouse/tasks/picking/{pickingSessionId}`

`POST /api/v1/wms/my-warehouse/tasks/picking/{pickingSessionId}/scan`

```json
{ "rawCode": "<tireUniqueId>" }
```

**متوقّع** `200`**:** `result=MATCH` → خط `PICKED`، إطار `OUTBOUND_PICKED`، موقع التخزين يُحرَّر.

### 10.11 إكمال الالتقاط والإرسال (OS-5)

`POST /api/v1/wms/my-warehouse/picking-sessions/{pickingSessionId}/complete` — `MANAGER_TOKEN`

**متوقّع** `200`**:** جلسة `COMPLETED`، طلب `PICKING_COMPLETED`.

`POST /api/v1/wms/my-warehouse/picking-sessions/{pickingSessionId}/dispatch`

**متوقّع** `200`**:** خطوط `SHIPPED`، إطارات `SHIPPED`، طلب `COMPLETED`.
...
| Generate picking      | `POST .../outbound-scheduling/generate-picking-sessions` | 201  | session `PENDING_APPROVAL`           |
| Approve picking       | `POST .../picking-sessions/{id}/approve`                 | 200  | outbound `PICKING_APPROVED`          |
| Pick scan             | `POST .../tasks/picking/{id}/scan`                       | 200  | tire `OUTBOUND_PICKED`               |
| Dispatch              | `POST .../picking-sessions/{id}/dispatch`                | 200  | outbound `COMPLETED`, tire `SHIPPED` |
| End state             | SQL inbound_request                                      | —    | `COMPLETED`                          |
...
## 8) ما لم يُختبر هنا (قادم — Phase 7)

- `@Scheduled` لتحرير الحجوزات المنتهية
- إشعارات manager/staff

---

## 9) Checklist سريع

- [x] V27 + V28 + V29 مطبّقة
...
- [ ] dashboard يعرض counters وجلسات بنسب التقدم